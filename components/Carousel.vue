<template>
  <div class="ar-carousel">
    <div class="ar-carousel__slides" ref="slides">
      <slot></slot>
    </div>
    <div class="ar-carousel__btn-left" @click="slide('left')">
      <ArrowLeftIcon />
    </div>
    <div class="ar-carousel__btn-right" @click="slide('right')">
      <ArrowRightIcon />
    </div>
  </div>
</template>
  ...
  methods: {
    slide(direction) {
      ...
      for (var i = 0; i < this.$refs.slides.childElementCount; i++) {
        this.$refs.slides.children[i].style = `transform: translateX(-${
          this.offsetSlide * 100
        }%);`
      }
    },
  },
