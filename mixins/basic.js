...
export default {
  ...
  methods: {
    ...
    async cloneConfirmed(item) {
      let itemCopy = { ...item }
      itemCopy.id = 'new'
      await this.$apollo.mutate({ mutation: this.mutation, variables: itemCopy, fetchPolicy: 'no-cache' })
      this.flush()
      this.getData()
    },
    remove(id) {
    ...
    async deleteConfirmed(id) {
      try {
        ...
        await this.$apollo.mutate({ mutation: this.deleteMutation, variables: { id }, fetchPolicy: 'no-cache' })
        this.flush()
        this.getData()
      } catch (e) {
